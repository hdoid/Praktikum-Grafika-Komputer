...
void drawSun() {
  // Buat efek denyut dengan menggunakan sin() yang berosilasi
  sunPulse = 1 + 0.2 * sin(frameCount * 0.05);  // nilai skala berubah-ubah
  
  fill(255, 223, 0);  // warna kuning untuk matahari
  noStroke();  // hilangkan garis tepi
  
  // Gunakan translate untuk memindahkan titik referensi ke posisi matahari
  pushMatrix();
  translate(width - 100, 100);  // pindah ke posisi matahari
  scale(sunPulse);  // tambahkan skala untuk efek denyut
  
  // Gambar matahari dengan skala yang berubah-ubah
  ellipse(0, 0, 80, 80);  // ukuran matahari diubah oleh scale
  popMatrix();  // kembalikan kondisi matriks ke asal
}
